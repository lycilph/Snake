...
namespace Snake
{
    public class NeuralNetworkBrain : Brain
    {
        ...
        */
        ...
        public NeuralNetworkBrain(Board board, Food food, Snake snake) : base(board, food, snake)
        {
        ...
        private (double, double, double) CheckDirection(Food food, int x, int y)
        {
            ...
            while (p.X >= 0 && p.X < board.BoardSizeInCells && p.Y >= 0 && p.Y < board.BoardSizeInCells)
            {
                ...
                if (snake.Hit(p) && distance_to_body == 0)
                    distance_to_body = (double)distance / board.BoardSizeInCells;

                p.Add(x, y);
                distance += diff;
            }

            distance_to_edge = (double)distance / board.BoardSizeInCells;
